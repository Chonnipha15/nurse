...
<script>
  // รับค่า Configuration ที่ส่งต่อมาจาก PHP
  const SUPABASE_URL = <?= json_encode($supabaseUrl) ?>;
  const SUPABASE_ANON_KEY = <?= json_encode($supabaseAnonKey) ?>;
  const supabase = window.supabase ? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;

  // รับข้อมูลสินค้าที่ PHP เรนเดอร์เป็น JSON ให้ JS นำไปจัดการตะกร้าต่อได้ทันที
  let catalog = <?= json_encode($catalog, JSON_UNESCAPED_UNICODE) ?>;
  let productIndex = {};
  catalog.forEach(g => g.items.forEach(p => productIndex[p.id] = p));
  ...
  async function simulateAdminConfirm() {
    ...
    // ซิงค์คำสั่งซื้อที่ยืนยันแล้วเข้าตาราง orders ใน Supabase
    if (supabase) {
      try {
